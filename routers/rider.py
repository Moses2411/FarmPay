"""
routers/admin.py
Admin endpoints for managing dispatch riders and disputes
"""
...
def create_dispatch_rider(
    ...
    # Return rider details with ID for frontend use
    return {
...
@router.get("/disputes/all", response_model=list[DisputeResponse])
...
def get_all_farmers(
    ...
    return {"role": "farmer", "count": farmers}
...
@router.get("/stats/farmer-profiles")
def get_all_sellers(
    db: Session = Depends(get_db), 
    admin: User = Depends(get_current_admin)
):
    farmer_profile = db.query(FarmerProfile).count()
    return {"total_farmer_profiles": farmer_profile}
...
@router.get("/dispatch-riders", response_model=List[dict])
def get_all_dispatch_riders(
    db: Session = Depends(get_db),
    admin: User = Depends(get_current_admin)
):
    """Get all dispatch riders with their details and order statistics"""
    riders = db.query(User).filter(User.role == "dispatch_rider").all()
